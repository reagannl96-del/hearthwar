...
for (let h = 1; h <= days * 24; h++) {
  ...
  if (h % 24 === 0) {
    ...
    { const sizes = Object.values(w.tribes).map((t) => t.members.length).sort((a, b) => b - a); const alone = list.filter((p) => !p.eliminated && p.tribeId === null).length; const hIn = humans.filter((hp) => hp.tribeId !== null).length; const hInv = humans.reduce((a, hp) => a + Object.values(w.tribes).filter((t) => t.invites?.some((i) => i.pid === hp.id)).length, 0); const pairs = { enemy: 0, ally: 0, nap: 0 } as Record<string, number>; for (const t of Object.values(w.tribes)) for (const d of Object.values(t.diplomacy ?? {})) pairs[d] = (pairs[d] ?? 0) + 1; console.log(`   diplomacy: ${JSON.stringify(pairs)}`); console.log(`   tribes: ${sizes.length} sizes [${sizes.slice(0, 12).join(',')}] tribeless AIs ${alone}, humans in tribes ${hIn}, invites open to humans ${hInv}`); }
    console.log(`   answering attacks: ${JSON.stringify(incidentStats)}`);
    { const all = Object.values(w.players).filter((p) => p.kind === 'ai'); const alive = all.filter((p) => !p.eliminated); const left = all.filter((p) => p.eliminated && p.villages.length === 0).length; console.log(`   population: ${alive.length} alive, ${all.length} ever, ${left} gone | newest: ${all.slice(-6).map((p) => p.name).join(', ')}`); }
